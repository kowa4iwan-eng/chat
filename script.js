...
function isOnline(user) { return typeof user?.online === 'boolean' ? user.online : Boolean(user?.active && Date.now() - Number(user.lastSeen || 0) < state.onlineTimeout); }
...
function setAvatar(el, user) {
  el.innerHTML = user?.avatar?.dataUrl ? `<img src="${user.avatar.dataUrl}" alt="">` : initials(user?.name || '?');
}
function toast(message) { els.toast.textContent = message; els.toast.classList.remove('hidden'); clearTimeout(toast.timer); toast.timer = setTimeout(() => els.toast.classList.add('hidden'), 2800); }
...
function readImageFile(file, maxMb = 2) {
  return new Promise((resolve, reject) => {
    if (!file) return resolve(null);
    if (file.size > maxMb * 1024 * 1024) return reject(new Error(`Фото має бути до ${maxMb} МБ`));
...
async function boot() {
  buildEmojiPicker();
  if (!state.sessionId) return showLogin();
  try { const data = await api('/api/bootstrap'); applyBootstrap(data); connectSocket(); }
  catch { localStorage.removeItem(SESSION_KEY); state.sessionId = null; showLogin(); }
}
function showLogin() { els.loginScreen.classList.remove('hidden'); els.appShell.classList.add('hidden'); setStatus('Очікує входу'); }
function showApp() { els.loginScreen.classList.add('hidden'); els.appShell.classList.remove('hidden'); }

function connectSocket() {
  if (typeof io !== 'function') return setStatus('Запусти сайт через Node server, не через GitHub Pages.');
  ...
  state.socket.on('bootstrap', (data) => { applyBootstrap(data); });
  state.socket.on('presence:update', (presence) => {
    const map = new Map(presence.map((p) => [p.id, p]));
    state.users = state.users.map((u) => ({ ...u, ...(map.get(u.id) || {}) }));
    renderOnlineCount(); renderConversationList(); renderChatHeader();
  });
...
async function refreshFromServer() {
  try { const data = await api('/api/bootstrap'); state.currentUser = data.me; state.users = data.users || []; state.groups = data.groups || []; state.messages = data.messages || []; renderApp(); renderChat(); }
  catch { toast('Не вдалося оновити дані'); }
}
...
function getLastMessage(conversationId) { const list = state.messages.filter((m) => m.conversationId === conversationId); return list[list.length - 1]; }

function renderConversationList() {
  const q = els.searchInput.value.trim().toLowerCase(); const user = state.currentUser; if (!user) return;
  if (state.activeTab === 'admin') { els.conversationList.innerHTML = `<button class="conversation" data-open-admin="true"><div class="avatar">⚙</div><div><h4>Адмін-панель</h4><p>Користувачі, групи, права</p></div><span></span></button>`; return; }
  let html = '';
...
function isMessageInActiveChat(m) { return m.conversationId === activeConversationId(); }
function renderChatHeader() {
  if (!state.activeChat) return;
  if (state.activeChat.type === 'direct') { const p = getUser(state.activeChat.id); if (!p) return; setAvatar(els.chatAvatar, p); els.chatTitle.textContent = p.name; els.chatSubtitle.textContent = isOnline(p) ? 'онлайн зараз' : 'не в мережі'; }
  else { const g = getGroup(state.activeChat.id); if (!g) return; els.chatAvatar.innerHTML = '#'; els.chatTitle.textContent = g.name; els.chatSubtitle.textContent = `${g.members.length} учасників`; }
}
function renderChat() {
  ...
  renderChatHeader(); els.emptyState.classList.add('hidden'); els.messagesArea.classList.remove('hidden'); els.composer.classList.remove('hidden'); document.querySelector('.attach-btn').classList.toggle('hidden', !can(state.currentUser, 'media'));
  const rows = state.messages.filter((m) => m.conversationId === activeConversationId()).map(renderMessage).join('');
  els.messagesArea.innerHTML = `<div class="day-separator">${formatDate()}</div>${rows}`; els.messagesArea.scrollTop = els.messagesArea.scrollHeight;
}
function renderMessage(m) {
  const sender = getUser(m.senderId) || { name: 'Невідомий' }; const mine = m.senderId === state.currentUser.id;
  let media = ''; if (m.media?.dataUrl) media = m.media.type?.startsWith('video/') ? `<video class="media-preview" src="${m.media.dataUrl}" controls></video>` : `<img class="media-preview" src="${m.media.dataUrl}" alt="Фото" />`;
...
function sendMessage() {
  ...
  if (!state.activeChat) return toast('Спочатку вибери чат'); if (!state.socket?.connected) return toast('Немає підключення до сервера'); if (!text && !media) return;
  state.socket.emit('message:send', { type: state.activeChat.type, targetId: state.activeChat.id, text, media }, (r) => { if (!r?.ok) return toast(`Не відправлено: ${r?.error || 'помилка'}`); els.messageInput.value = ''; els.fileInput.value = ''; els.messageInput.placeholder = 'Напиши повідомлення...'; state.pendingFile = null; });
}
function buildEmojiPicker() { els.emojiPicker.innerHTML = emojis.map((e) => `<button type="button" data-emoji="${e}">${e}</button>`).join(''); }
function openAdminPanel() { if (!can(state.currentUser, 'admin')) return toast('Адмін-панель доступна тільки адміну'); renderAdminPanel(); els.adminPanel.classList.remove('hidden'); els.sidebar.classList.remove('open'); }
...
async function createUser(event) {
  ...
  try {
    const avatar = await readImageFile($('#newUserAvatar').files[0], 2);
    await api('/api/users', { method: 'POST', body: JSON.stringify({ name: $('#newUserName').value.trim(), login: $('#newUserLogin').value.trim().toLowerCase(), password: $('#newUserPassword').value.trim(), role: $('#newUserRole').value.trim() || 'Користувач', avatar, rights: { direct: $('#rightDirect').checked, groups: $('#rightGroups').checked, media: $('#rightMedia').checked, seeOnline: $('#rightSeeOnline')?.checked ?? true, admin: $('#rightAdmin').checked } }) });
    els.userForm.reset(); $('#rightDirect').checked = true; $('#rightGroups').checked = true; $('#rightMedia').checked = true; if ($('#rightSeeOnline')) $('#rightSeeOnline').checked = true; await refreshFromServer(); toast('Користувача створено');
...
async function saveUser(userId, clearAvatar = false) {
  ...
  try {
    ...
    const avatar = clearAvatar ? null : await readImageFile(avatarFile, 2);
    const body = { name: card.querySelector('[data-edit="name"]').value.trim(), login: card.querySelector('[data-edit="login"]').value.trim().toLowerCase(), role: card.querySelector('[data-edit="role"]').value.trim(), active: card.querySelector('[data-active]').checked, rights: {} };
    ...
    card.querySelectorAll('[data-right]').forEach((input) => { body.rights[input.dataset.right] = input.checked; });
    await api(`/api/users/${userId}`, { method: 'PATCH', body: JSON.stringify(body) });
    await refreshFromServer(); toast('Користувача оновлено');
  } catch (e) { toast(`Помилка: ${e.message}`); }
}
async function createGroup(event) { event.preventDefault(); try { const members = Array.from(els.groupMembers.querySelectorAll('input:checked')).map((i) => i.value); await api('/api/groups', { method: 'POST', body: JSON.stringify({ name: $('#newGroupName').value.trim(), members }) }); els.groupForm.reset(); await refreshFromServer(); toast('Групу створено'); } catch (e) { toast(`Помилка: ${e.message}`); } }
async function logout() { try { await api('/api/logout', { method: 'POST' }); } catch {} if (state.socket) state.socket.disconnect(); localStorage.removeItem(SESSION_KEY); state.sessionId = null; state.currentUser = null; state.activeChat = null; showLogin(); }

els.loginForm.addEventListener('submit', async (event) => { event.preventDefault(); try { const data = await api('/api/login', { method: 'POST', body: JSON.stringify({ login: els.loginInput.value.trim(), password: els.passwordInput.value.trim() }) }); state.sessionId = data.sessionId; localStorage.setItem(SESSION_KEY, state.sessionId); els.loginForm.reset(); applyBootstrap(data); connectSocket(); } catch { toast('Невірний логін або пароль, або сервер не запущений'); } });
...
els.fileInput.addEventListener('change', async () => { try { const file = els.fileInput.files[0]; if (!file) return; if (!can(state.currentUser, 'media')) { els.fileInput.value = ''; return toast('Адмін вимкнув тобі відправку фото/відео'); } state.pendingFile = await readImageFile(file, 6); els.messageInput.placeholder = `Прикріплено: ${file.name}`; toast('Файл прикріплено. Натисни відправити.'); } catch (e) { els.fileInput.value = ''; toast(e.message); } });
